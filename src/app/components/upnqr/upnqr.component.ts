...
export class UpnqrComponent implements OnInit {
	...
	createJsZip(): void {
		...
		this.selectedOsebas.forEach((oseba: Oseba) => {
			this.dataService
				...
				.subscribe((data: Blob) => {
					const fileName = this.removeSloCharacters(oseba.imePlacnik.replace(/ /g, '')) + '.pdf';
	...
	removeSloCharacters(rString: string): string {
		...
		for (var i = 0; i < defaultDiacriticsRemovalMap.length; i++) {
			rString = rString.replace(defaultDiacriticsRemovalMap[i].letters, defaultDiacriticsRemovalMap[i].base);
		}
